...
class Engine(object):
    ...
    def __init__(self, config):
        ...
        self.user_relation_graph_mlp = None
        self.user_relation_graph_gmf = None
        self.topk_user_relation_graph_mlp = None
        self.topk_user_relation_graph_gmf = None
    ...
    def fed_train_single_batch(self, model_client, batch_data, optimizers, user):
        ...
        reg_item_embedding_gmf = copy.deepcopy(self.server_model_param['embedding_item_gmf.weight'][user].data)
        ...
        if self.config['use_cuda'] is True:
            ...
            reg_item_embedding_gmf = reg_item_embedding_gmf.cuda()
        optimizer.zero_grad()
        ...
        regularization_term_gmf = compute_regularization_gmf(model_client, reg_item_embedding_gmf)
        loss += (self.config['reg'] * regularization_term_mlp + self.config['reg_gmf'] * regularization_term_gmf)
        loss.backward()
        optimizer.step()
    ...
    def aggregate_clients_params(self, round_user_params, round):
        ...
        if round % 1 == 0:
            if self.config['construct_graph_source'] == 'item':
                if self.user_relation_graph_mlp is not None:
                    del self.user_relation_graph_mlp
                self.user_relation_graph_mlp = construct_user_relation_graph_via_item_mlp(round_user_params, self.config['num_items'],
                                                                self.config['latent_dim'],
                                                                self.config['similarity_metric'])
                if self.user_relation_graph_gmf is not None:
                    del self.user_relation_graph_gmf
                self.user_relation_graph_gmf = construct_user_relation_graph_via_item_gmf(round_user_params, self.config['num_items'],
            ...
            if self.topk_user_relation_graph_mlp is not None:
                del self.topk_user_relation_graph_mlp
            self.topk_user_relation_graph_mlp = select_topk_neighboehood(self.user_relation_graph_mlp, self.config['neighborhood_size'],
                                                                self.config['neighborhood_threshold'])
            if self.topk_user_relation_graph_gmf is not None:
                del self.topk_user_relation_graph_gmf
            self.topk_user_relation_graph_gmf = select_topk_neighboehood(self.user_relation_graph_gmf, self.config['neighborhood_size'],
                                                                self.config['neighborhood_threshold'])
        ...
        updated_item_embedding_mlp = MP_on_graph_mlp(round_user_params, self.config['num_items'], self.config['latent_dim'],
                                             self.topk_user_relation_graph_mlp, self.config['mp_layers'])
        updated_item_embedding_gmf = MP_on_graph_gmf(round_user_params, self.config['num_items'], self.config['latent_dim_gmf'],
                                                     self.topk_user_relation_graph_gmf, self.config['mp_layers'])
    ...
    def fed_train_a_round(self, all_train_data, round_id):
        ...
        if round_id == 0:
            ...
            for user in participants:
                ...
                self.server_model_param['embedding_item_gmf.weight'][user] = copy.deepcopy(self.model.state_dict()['embedding_item_gmf.weight'].data.cpu())
        ...
        for user in participants:
            ...
            # round_participant_params[user]['embedding_item_mlp.weight'] += Laplace(0, self.config['dp']).expand(round_participant_params[user]['embedding_item_mlp.weight'].shape).sample()
            # round_participant_params[user]['embedding_item_gmf.weight'] += Laplace(0, self.config['dp']).expand(round_participant_params[user]['embedding_item_gmf.weight'].shape).sample()
        ...
        self.aggregate_clients_params(round_participant_params, round_id)
    ...
    def fed_evaluate(self, evaluate_data):
        ...
        for user in range(self.config['num_users']):
            ...
            # user_param_dict['embedding_item_mlp.weight'] = copy.deepcopy(self.server_model_param['embedding_item_mlp.weight'][user].data.cuda())
